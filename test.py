...
def parse_arguments():
    ...
    parser.add_argument('--view', type=str, default='wrist-rgb', 
...
def main():
    ...
    if args.rlbench:
        import rlbench.gym
        if args.normalize:
            env_fn = lambda: Normalize_Observation(RLBench_Wrapper(gym.make(args.env, render_mode='rgb_array', max_episode_length = args.max_ep_len, dense_rewards = args.rewards), args.view))
        else:
            env_fn = lambda: RLBench_Wrapper(gym.make(args.env, render_mode='rgb_array', max_episode_length = args.max_ep_len, dense_rewards = args.rewards), args.view)
    elif args.normalize:
